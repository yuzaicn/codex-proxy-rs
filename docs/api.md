...
| 409 | `40901` | 资源状态冲突 |
| 429 | `42901` | 登录尝试过多 |
| 429 | `42902` | 上游限流；可安全重试 |
| 500 | `50001` | 服务内部错误 |
| 502 | `50201` | 上游服务请求失败 |
| 502 | `50202` | 不可逆上游操作的执行结果未知；刷新状态后再决定是否重试 |
| 503 | `50301` | 依赖服务暂不可用；上游明确返回 5xx 时文案为“上游服务暂不可用，请稍后重试” |

上游返回 `429` 时，错误信封使用 `42902` 与文案“上游限流，请稍后重试”。若上游同时提供合法的
`Retry-After` 秒数，响应会将其钳制到最多 300 秒，并同时写入标准 `Retry-After` 响应头与
`data.retryAfterSeconds`；未提供时 `data` 保持 `null`。登录限流继续使用 `42901`，不会携带这份上游
重试信息。
...
| `POST` | `/api/admin/accounts/import` | `{ provider, data, settings?, outboundProxyId? }`；响应 `{ importedCount, createdCount, updatedCount, accountIds, failures: [{ index, code, retryable, message }] }`（`index` 为 `accounts` 数组 0-based 下标） | 导入或按上游身份更新账号，可同时应用调度、分组设置与默认代理；合法文档（包括全部凭据失败）返回 200，单项失败不影响其它账号；`code` 当前包括 `refresh_rejected`、`account_banned`、`refresh_unavailable`、`refresh_rate_limited`、`refresh_upstream_failed`、`refresh_upstream_unavailable`、`refresh_ambiguous`、`refresh_lease_unavailable`、`invalid_credential`、`invalid_input`、`missing_refresh_token`、`not_found`、`personal_access_token_validation_failed`，该集合未来可扩展；调用方应以 `retryable` 为重试依据并容忍未知 code。串行导入耗时随账号数和上游延迟线性增长，直连调用方应为大批次预留请求超时预算 |
...
| `POST` | `/api/admin/accounts/rotate` | `{ provider: "openai", accountId, accessToken, refreshToken?, idToken? }` | 手工替换 OpenAI OAuth token；仅支持 OpenAI，省略可选 token 字段时保留现有值 |
...
`POST /api/admin/accounts/rotate` 成功返回 `{ accountId }`；`refreshToken` 与 `idToken` 为可选字段，且该接口仅接受 `provider: "openai"`。
