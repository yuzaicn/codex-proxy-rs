...
| `POST` | `/api/admin/accounts/import` | `{ provider, data, settings?, outboundProxyId? }`；响应 `{ importedCount, insertedCount, updatedCount, accountIds, failures: [{ index, kind }] }` | 导入或按上游身份更新账号，可同时应用调度、分组设置与默认代理；单项凭据失败不影响其它账号，`kind` 为 `invalid_credential`、`unavailable` 或 `ambiguous` |
