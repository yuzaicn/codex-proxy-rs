...
| `POST` | `/api/admin/accounts/import` | `{ provider, data, settings?, outboundProxyId? }`；响应 `{ importedCount, insertedCount, updatedCount, accountIds, failures: [{ index, code, retryable, message }] }`（`index` 为 `accounts` 数组 1-based 输入序号） | 导入或按上游身份更新账号，可同时应用调度、分组设置与默认代理；合法文档中的单项凭据失败不影响其它账号，失败项返回稳定错误码与安全提示 |
