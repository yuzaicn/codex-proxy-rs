...
| `POST` | `/api/admin/accounts/import` | `{ provider, data, settings?, outboundProxyId? }`；响应 `{ importedCount, createdCount, updatedCount, accountIds, failures: [{ index, code, retryable, message }] }`（`index` 为 `accounts` 数组 0-based 下标） | 导入或按上游身份更新账号，可同时应用调度、分组设置与默认代理；合法文档中的单项凭据失败不影响其它账号，失败项返回稳定错误码与安全提示 |
