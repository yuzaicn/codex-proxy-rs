...
interface ResetCreditsSession {
  ...
  creditsObservedListeners: Set<(accountId: string, availableCount: number) => void>
}
...
function getResetCreditsSession(accountId: string) {
  ...
  if (!session) {
    session = shallowReactive<ResetCreditsSession>({
      ...
      creditsObservedListeners: new Set(),
    })
...
// 会话内拿到新的可用张数（手动查询或消费扣减）时，通知仍订阅该账号的实例，供调用方同步列表列上的观测值。
function notifyCreditsObserved(session: ResetCreditsSession) {
  const snapshot = session.snapshot
  if (!snapshot)
    return
  for (const listener of session.creditsObservedListeners)
    listener(session.accountId, snapshot.availableCount)
}
...
export function useAccountResetCredits(options: {
  ...
  watch(
    ...
    (accountId, _, onCleanup) => {
      ...
      const observedListener = (accountId: string, availableCount: number) =>
        options.onCreditsObserved?.(accountId, availableCount)
      target.creditsObservedListeners.add(observedListener)
      onCleanup(() => {
        target.accountUpdatedListeners.delete(listener)
        target.creditsObservedListeners.delete(observedListener)
      })
    },
